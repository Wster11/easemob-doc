...
const DOC_PATH = path.resolve(__dirname, '../../../v5/document')
...
function buildDocSidebar() {
  ...
  platformList.forEach(platform => {
    const key = `/v5/document/${platform}/`
...
function handleSidebarItem(platform, sidebar) {
  ...
  if (hasChildren) {
    ...
    if (linkExists(platform, sidebar.link)) {
      const newLink = `/v5/document/${platform}/${sidebar.link}`
      return {...sidebar, link:newLink}
    }
  }
}

export const DOC_SIDEBAR_V5 = buildDocSidebar()

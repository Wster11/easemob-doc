...
const CHAT_DOC_PATH = path.resolve(__dirname, "../../../v5/uikit/chatuikit");
const CHATROOM_DOC_PATH = path.resolve(__dirname, "../../../v5/uikit/chatroomuikit");
...
function buildChatUikitSidebar() {
  ...
  chatPlatformList.forEach((platform) => {
    const key = `/v5/uikit/chatuikit/${platform}/`;
...
function buildChatroomUikitSidebar() {
  ...
  chatroomPlatformList.forEach((platform) => {
    const key = `/v5/uikit/chatroomuikit/${platform}/`;
...
function handleSidebarItem(platform, sidebar, docPath, kitType) {
  ...
  if (hasChildren) {
    ...
    if (linkExists(platform, sidebar.link, docPath)) {
      const newLink = `/v5/uikit/${kitType}/${platform}/${sidebar.link}`;
      return { ...sidebar, link: newLink };
    }
  }
}

export const CHAT_UIKIT_SIDEBAR_V5 = buildChatUikitSidebar();
export const CHATROOM_UIKIT_SIDEBAR_V5 = buildChatroomUikitSidebar();

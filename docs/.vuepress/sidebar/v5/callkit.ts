...
const CALL_DOC_PATH = path.resolve(__dirname, "../../../v5/callkit");
...
function handleSidebarItem(platform, sidebar, docPath, kitType) {
  ...
  if (hasChildren) {
    ...
    if (linkExists(platform, sidebar.link, docPath)) {
      const newLink = `/v5/${kitType}/${platform}/${sidebar.link}`;
      return { ...sidebar, link: newLink };
    }
  }
}

export const CALL_KIT_SIDEBAR_V5 = buildCallKitSidebar();
